...
#define LOG_TAG "NativeWalker"
void setAccelerometer(int8_t x, int8_t y, int8_t z) {
    // Log the incoming accelerometer values
    __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, "Accelerometer set: x=%d, y=%d, z=%d", x, y, z);

    // Chip ID (0x02 for the accelerometer in the Pokewalker)
    accel.memory[0] = 0x02;

    // X, Y, Z values (8-bit each)
    accel.memory[1] = x;  // X axis
    accel.memory[2] = y;  // Y axis
    accel.memory[3] = z;  // Z axis
...
int runNextInstruction(uint64_t* cycleCount){
    //__android_log_print(ANDROID_LOG_INFO, "WalkerC", "PC: %04X, opcode: %02X", pc, memory[pc]);
...
// Modified initWalker to accept AAssetManager
void initWalker(AAssetManager* assetManager) {
    ...
    eeprom.memory = malloc(EEPROM_SIZE); // EEPROM_SIZE should be 64 * 1024
    memset(eeprom.memory, 0xFF, EEPROM_SIZE);

#ifndef INIT_EEPROM
    if (assetManager != NULL) {
        AAsset* eepromAsset = AAssetManager_open(assetManager, "eeprom.bin", AASSET_MODE_BUFFER);
        if (eepromAsset != NULL) {
            off_t assetLength = AAsset_getLength(eepromAsset);
            // Ensure you don't read more than your buffer can hold
            size_t readSize = (assetLength > EEPROM_SIZE) ? EEPROM_SIZE : (size_t)assetLength;

            int bytesRead = AAsset_read(eepromAsset, eeprom.memory, readSize);
            if (bytesRead < 0) {
                __android_log_print(ANDROID_LOG_ERROR, "WalkerC", "Failed to read eeprom.bin");
            } else {
                __android_log_print(ANDROID_LOG_INFO, "WalkerC", "Read %d bytes from eeprom.bin", bytesRead);
            }
            AAsset_close(eepromAsset);
        } else {
            __android_log_print(ANDROID_LOG_ERROR, "WalkerC", "Failed to open eeprom.bin from assets");
        }
    } else {
        __android_log_print(ANDROID_LOG_ERROR, "WalkerC", "AssetManager is NULL in initWalker");
    }
#endif
    ...
    // For "rom.bin"
    if (assetManager != NULL) {
        AAsset* romAsset = AAssetManager_open(assetManager, "rom.bin", AASSET_MODE_BUFFER);
        if (romAsset != NULL) {
            off_t romSize = AAsset_getLength(romAsset);
            // Ensure 'memory' is large enough and you read correctly
            if (romSize > 0 && romSize <= MEM_SIZE) { // MEM_SIZE should be your total allocated memory size
                int bytesRead = AAsset_read(romAsset, memory, romSize); // Read directly into 'memory' if it's the target
                if (bytesRead < 0) {
                    __android_log_print(ANDROID_LOG_ERROR, "WalkerC", "Failed to read rom.bin");
                } else {
                    __android_log_print(ANDROID_LOG_INFO, "WalkerC", "Read %d bytes from rom.bin", bytesRead);
                }
            } else if (romSize > MEM_SIZE) {
                __android_log_print(ANDROID_LOG_ERROR, "WalkerC", "rom.bin is larger than allocated memory buffer");
            }
            AAsset_close(romAsset);
        } else {
            __android_log_print(ANDROID_LOG_ERROR, "WalkerC", "Failed to open rom.bin from assets");
            // Handle the error: your app might not function without the ROM
        }
    } else {
        __android_log_print(ANDROID_LOG_ERROR, "WalkerC", "AssetManager is NULL for rom.bin loading");
    }

    // After loading rom.bin into memory, and before pc = entry;
    __android_log_print(ANDROID_LOG_INFO, "WalkerC", "ROM[0x%04X]=%02X, ROM[0x%04X]=%02X...", entry, memory[entry], entry+1, memory[entry+1]);
